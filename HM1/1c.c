...
int row_number = 0;      //row number for bag of tasks, task is one row to calculate
pthread_mutex_t mutex_row_number;  //mutex for locking the row number to upd it atomically
...
void *Worker(void *arg) {
  ...
  //loop until there is no more tasks, eg all rows were processed
  while(true){
  /* determine the row we have to do from a bag of tasks, lock it for it to be atomic*/
  pthread_mutex_lock(&mutex_row_number);
  i = row_number++;  
  pthread_mutex_unlock(&mutex_row_number);
